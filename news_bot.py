        ...
        try:
            ...
            for entry in feed.entries[:3]:
                ...
                if entry_id and entry_id not in sent_news_ids:
                    ...
                    link  = entry.get("link", "")
                    messages.append(f"📰 <b>{feed_info['name']}</b>\n{title}\n<a href='{link}'>Read more</a>")
        except Exception as e:
...
# ─── Feature 2: Market Open Summary at 9:08 AM ──────────────
def market_open_summary():
    logger.info("Sending market open summary...")
    try:
        # Nifty 50 via Yahoo Finance
        ...
        msg = (
            ...
            f"Market is {'<b>GAP UP</b> 📈' if nifty_chg > 100 else '<b>GAP DOWN</b> 📉' if nifty_chg < -100 else '<b>FLAT</b> ➡️'}"
        )
        send_message(msg)
    except Exception as e:
...
# ─── Feature 3: Nifty 500 Stock Mover Alerts (+/-2%) ────────
NIFTY500_SYMBOLS = [
...
def check_stock_movers():
    ...
    for symbol in NIFTY500_SYMBOLS:
        try:
            ...
            # Alert if +/-2% AND volume is at least 1.2x average
            if abs(change) >= 2.0 and vol_ratio >= 1.2:
                last_alert = alerted_stocks.get(symbol, 0)
                now_ts = time.time()
                if now_ts - last_alert > 3600:  # alert once per hour per stock
...
# ─── Feature 4: Nifty Reversal / -250 Alert ─────────────────
...
def check_nifty_reversal():
    ...
    try:
        ...
        # Reset -250 alert if nifty recovers above -150
        if change_pts > -150:
            nifty_low_alerted = False

        # Alert 2: Reversal detection
        # If we have 6 readings and last 3 are rising after a fall
        if len(prev_nifty_readings) == 6:
...
def conditional_news():
    fetch_market_news()

def conditional_movers():
    if is_market_hours():
        check_stock_movers()

def conditional_reversal():
    if is_market_hours():
        check_nifty_reversal()

def setup_schedule():
    # Feature 1: News every 30 mins from 8 AM to 6 PM
    schedule.every(30).minutes.do(conditional_news)

    # Feature 2: Market open at 9:08 AM
    schedule.every().day.at("09:08").do(market_open_summary)

    # Feature 3: Stock movers every 15 mins during market hours
    schedule.every(15).minutes.do(conditional_movers)

    # Feature 4: Nifty reversal check every 5 mins
    schedule.every(5).minutes.do(conditional_reversal)
